...
$min= $mysql->query("SELECT min(number) FROM queue");
$min=$min-> fetch_assoc();
$min=reset($min);

$name=$mysql->query ("SELECT*  FROM `queue` WHERE  `number` = '$min'");
$name=$name-> fetch_assoc();
$n=end($name);
echo("сейчас в очереди ");
echo($n);
?>
...
<?php
 ...
 if( isset( $_POST['next'] ) )
 {
$mysql->query ("DELETE FROM `queue` WHERE number = '$min'");

$mysql->query("UPDATE `queue` SET `number`=`number`-1");
